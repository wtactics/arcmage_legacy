...
/* Region: victory points widget */

/* Template of victory points number */
function changeTooltip(e) {
    return "<div class='victoryCounter'><span>" + e.value + "</span></div>";
}

/* When the player victory points are updated by the widget, store it in the model, and send the communication command */
function changePlayerVictoryPoints(e){
    vue.player.VictoryPoints = e.value;
    updatePlayerStatsAction(vue.player.playerGuid);
	return changeTooltip(e);
}

/* When the player victory points are updated by the widget, store it in the model, and send the communication command */
function changeOpponentVictoryPoints(e){
	vue.opponent.VictoryPoints = e.value;
	updatePlayerStatsAction(vue.opponent.playerGuid);
	return changeTooltip(e);
}

/* EndRegion: victory points widget */
...
function resizeGame() {
	...
	var radious = (distance / 2.0) *0.5;
	var sliderWidth = radious * 0.15;
	
	var opponentAngle = 135;
	var angle = toRadians(-opponentAngle);
	var opponentTop =  (1 + Math.sin(angle)) * radious;
	var opponentLeft = (1+ Math.cos(angle)) * radious;
	
	$("#opponentVicoryPoints").offset({
		left: -opponentLeft,
		top: -opponentTop,
	});
	...
	$("#opponentVicoryPoints").roundSlider({
		handleShape: "square",
		radius: radious,
		value: vue.opponent.VictoryPoints,
		circleShape: "custom-half",
		mouseScrollAction: true,
		editableTooltip: false,
		max: 30,
		sliderType: "min-range",
		startAngle: opponentAngle,
		width: sliderWidth,
		tooltipFormat: "changeOpponentVictoryPoints"
	});
	
	
	var playerAngle = -45;
	angle = toRadians(-playerAngle);
	var playerTop =  height - radious*2.0 + opponentLeft;
	var playerLeft = width - radious *2.0 + opponentTop;
	
	
	$("#playerVicoryPoints").offset({
		left: playerLeft,
		top: playerTop,
	});
	...
	$("#playerVicoryPoints").roundSlider({
		handleShape: "square",
		radius: radious,
		value: vue.player.VictoryPoints,
		circleShape: "custom-half",
		mouseScrollAction: true,
		editableTooltip: false,
		max: 30,
		...
		width: sliderWidth,
		tooltipFormat: "changePlayerVictoryPoints"
	});
}
