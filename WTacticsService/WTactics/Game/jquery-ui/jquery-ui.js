...
var intersect = $.ui.intersect = ( function() {
	...
	return function( draggable, droppable, toleranceMode, event ) {
		...
		case "perspectiveintersect":
		
		/*
			var dragWidth = draggable.element.width();
			var dragHeight = draggable.element.height();
		*/
			var dragBoundingBox = draggable.element[0].getBoundingClientRect();
			x1 = dragBoundingBox.left;
			y1 = dragBoundingBox.top;
			x2 = dragBoundingBox.right;
			Y2 = dragBoundingBox.bottom;
			
		
			var dropBoundingBox = droppable.element[0].getBoundingClientRect();
			l = dropBoundingBox.left;
			t = dropBoundingBox.top;
			r = dropBoundingBox.right;
			b = dropBoundingBox.bottom;
		
			
			
		/*
			console.log('draggable ' + x1 + ',' + y1 + ' to ' + x2 +',' +y2 );
			console.log('droppable ' + l + ',' + t + ' to ' + r +',' + b );
		*/
		
			var trans = vue.inverseScreenTransformMatrix;
		
			var result = numeric.dot(trans, [x1, y1, 0, 1]);
			var px1 = result[0]/result[3];
			var py1 = result[1]/result[3];
			
			result = numeric.dot(trans, [x2, y2, 0, 1]);
			var px2 = result[0]/result[3];
			var py2 = result[1]/result[3];
			
			result = numeric.dot(trans, [l, t, 0, 1]);
			var pl = result[0]/result[3];
			var pt = result[1]/result[3];
			
			result = numeric.dot(trans, [r, b, 0, 1]);
			var pr = result[0]/result[3];
			var pb = result[1]/result[3];
		...
		*/
			var rightHalf =   px1 + ( pwidth / 2 ) > pl;
			var leftHalf = px2 - ( pwidth / 2  ) < pr;
			var topHalf =  py1 + ( pheight /2  ) > pt;
			var bottomHalf = py2 - ( pheight /2 ) < pb;
		
			var isPerspectiveIntersect = rightHalf && leftHalf && topHalf && bottomHalf; 
/*			
			console.log('dropable dim ' + pwidth + ',' + pheight);
			console.log('draggable ' + px1 + ',' + py1 + ' to ' + px2 +',' +py2 );
			console.log('droppable ' + pl + ',' + pt + ' to ' + pr +',' + pb );
			console.log(isPerspectiveIntersect);
*/		
			return isPerspectiveIntersect;
